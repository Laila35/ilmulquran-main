...
// Course data
  const regularCourses = [
    ...
    { id: 10, courseName: "Quranic Arabic Course", link: "/quranic-arabic-course" },
  ];
        ...
        {item.dropdown ? (
          <>
            ...
            >
              ...
              {item.dropdown && <ChevronDownIcon className="w-3 h-3" />}
            </button>
            {activeDropdown === item.id && (
              <div className={`absolute z-20 bg-teal-400 text-white rounded-lg shadow-lg mt-2 ${item.dropdown.className}`}>
          ...
          <Link
            href={item.href}
            className="hover:text-teal-200 font-medium text-sm transition duration-200 flex items-center space-x-2"
          >
            <item.icon className="w-4 h-4" />
            <span>{item.label}</span>
          </Link>
